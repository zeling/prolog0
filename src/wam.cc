...
template <typename T> using varmap = std::unordered_map<std::string, T>;

template <typename T> using termmap = std::unordered_map<const term *, T>;
...
struct compile_visitor : ast_visitor<compile_visitor<Compile>> {
    ...
    compile_visitor(inst_stream &o, const structure *top, varset varseen)
        : _top(top), _o(o), _varseen(std::move(varseen)) {
        reg_t arg = 1;
        for (auto &t : top->args) {
            _args[t.get()] = arg++;
        }
    ...
    void visit_variable(const variable *v) {
        ...
        if (_args.find(v) != _args.end()) {
            if (seen)
                static_cast<Compile *>(this)->emit_seen_variable_argument(
                    v, _args[v]);
            else
                static_cast<Compile *>(this)->emit_nonseen_variable_argument(
                    v, _args[v]);
        } else {
            if (seen)
...
termmap<reg_t> flatten(const structure *s, varmap<reg_t> *env) {
    ...
    struct visitor : ast_visitor<visitor> {
        ...
        visitor(reg_t start, const structure *s, varmap<reg_t> *env)
...
varset compile_query_term(inst_stream &_o, const structure *s, varset seen,
    ...
    struct compile : compile_visitor<compile> {
        ...
        compile(termmap<reg_t> f, inst_stream &o, const structure *s,
                varset seen)
            : compile_visitor(o, s, std::move(seen)), _flattened(std::move(f)) {
        }
        ...
        void emit_toplevel(const structure *s) { EMIT_INST(call, s->functor); }
...
varset compile_program_term(inst_stream &o, const structure *s, varset seen,
    ...
    struct compile : compile_visitor<compile> {
        ...
        std::unordered_map<const term *, reg_t> _flattened;
        compile(std::unordered_map<const term *, reg_t> f, inst_stream &o,
                const structure *s, varset seen)
        ...
        void emit_toplevel(const structure *s) { EMIT_INST(proceed); }
...
std::unordered_set<std::string> get_names(const term *t) {
    struct visitor : ast_visitor<visitor> {
        ...
        void visit_variable(const variable *v) { _met.insert(v->name); }
    } v;
    v.walk_preorder(t);
    return v._met;
}
...
void compile_query(inst_stream &_o, const query *qry) {
    ...
    auto env = analyze_environment(std::move(first), ++qry->terms.begin(),
                                   qry->terms.end());
    size_t env_size = env.size();
    if (env_size)
        EMIT_INST(allocate, env_size);
    varset seen;
    for (auto &t : qry->terms) {
        seen = compile_query_term(_o, t.get(), std::move(seen), &env);
    }
    if (env_size)
        EMIT_INST(deallocate);
}
...
void compile_rule(inst_stream &_o, const rule *r) {
    ...
    auto env =
        analyze_environment(std::move(first), ++r->tail.begin(), r->tail.end());
...
void compile_program(inst_stream &o, const program *prg) {
    struct visitor : ast_visitor<visitor> {
        ...
        void visit_rule(const rule *r) { compile_rule(_o, r); }

        void visit_fact(const fact *f) { compile_fact(_o, f); }
    } v{o};
    v.visit(prg);
}
