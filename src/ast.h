...
template <typename Visitor> struct term_visitor;
...
struct variable : public term {
    ...
    static bool classof(const term *t) { return t->kind() == term::variable; }
};
...
struct functor {
    ...
    functor(std::string name, size_t arity)
...
struct structure : public term {
    ...
    structure(std::string name, size_t arity,
              std::vector<std::unique_ptr<term>> args)
        : term(term::structure), args(std::move(args)),
          functor(std::move(name), arity) {}

    /* a special case where the structure represents a constant */
    bool is_constant() { return functor.arity == 0; }

    static bool classof(const term *t) { return t->kind() == term::structure; }
};
...
class program : ast {
  ...
  public:
    ...
    static bool classof(const ast *a) { return a->kind() == ast::program; }
};
...
struct fact : public program {
  ...
  public:
    fact(std::unique_ptr<structure> s)
        : program(program::fact), _str(std::move(s)) {}

    static bool classof(const program *p) { return p->kind() == program::fact; }
};
...
struct rule : public program {
  ...
  public:
    rule(std::unique_ptr<structure> head,
         std::vector<std::unique_ptr<structure>> tail)
        : program(program::rule), head(std::move(head)), tail(std::move(tail)) {
    }

    static bool classof(const program *p) { return p->kind() == program::rule; }
};

struct query : ast {
    std::vector<std::unique_ptr<structure>> terms;
    query(std::vector<std::unique_ptr<structure>> terms)
        : ast(ast::query), terms(std::move(terms)) {}

    static bool classof(const ast *a) { return a->kind() == ast::query; }
};

template <typename Visitor> struct ast_visitor {
#define DELEGATE_VISIT(what)                                                   \
    void visit(const what *x) { static_cast<Visitor *>(this)->visit_##what(x); }
...
#define EMPTY_VISIT(what)                                                      \
    void visit_##what(const what *x) {}
