...
std::unique_ptr<structure> parser::parse_structure() {
    ...
    if (next.type() == token::LPAREN) {
        args = parse_list([this] { return parse_term(); }, token(token::COMMA),
                          token(token::RPAREN));
    } else {
        _scanner.push_back(std::move(next));
    }
    return std::make_unique<structure>(first.literal(), args.size(),
                                       std::move(args));
}
...
std::unique_ptr<query> parser::parse_query() {
    ...
    auto body = parse_list([this] { return this->parse_structure(); },
                           token(token::COMMA), token(token::PERIOD));
...
std::unique_ptr<program> parser::parse_program() {
    ...
    if (t.type() == token::COLONDASH) {
        auto tail = parse_list([this] { return this->parse_structure(); },
                               token(token::COMMA), token(token::PERIOD));
